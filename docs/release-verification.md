...
|---|---|---|---|---|
| `prithvi_crop_classification_colab.ipynb` (`E2E`) | `0f1284d` / `7f88cfa0` | 2026-09-20 | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-prithvi-crop-classification` v2; image `torch 2.10.0+cu128` before the pinned install, `torch 2.14.0+cu130`, `tifffile 2026.9.15`, `numpy 2.5.3` after, Python 3.12.13, `cuda`) | **PASSED** — 11/11 code cells ok (1 restart after install cell); 134 files, 3509 MB fetched into a clean runtime (the Hub snapshot, the tarball and the 120 extracted members); comparison test mean IoU (majority baseline 0.011): frozen 0.4379 → adapted 0.4302, accuracy (baseline 0.137) 0.5905 → 0.5813, mean class accuracy 0.6239 → 0.6250 (best epoch 4, class-weighted validation loss 0.9368 → 0.9114, validation mean IoU 0.4609 → 0.4451); reload parity mean_iou_diff: 0.0, metrics_identical: True, max_abs_score_diff: 0.0; run summary and executed notebook archived under `.agent/backups/kaggle-e2e-2026-09-19/out/dimer-nb2-prithvi-crop-classification/v2/evidence/` in the workspace |
...
|---|---|---|---|---|---|
| 2026-09-20 | `0f1284d` / `7f88cfa0` | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-prithvi-crop-classification` v2; image `torch 2.10.0+cu128` before the pinned install, `torch 2.14.0+cu130`, `tifffile 2026.9.15`, `numpy 2.5.3` after, Python 3.12.13, `cuda`) | Default sample path, `Run all` from a fresh interpreter with an empty Hugging Face cache and no repository checkout (blob SHA-1 verified against GitHub before execution); the checkpoint audited and converted in the notebook, the tarball fetched and the pinned members extracted by the notebook | 278.6 s | **PASSED** — 11/11 code cells ok (1 restart after install cell); 134 files, 3509 MB fetched into a clean runtime (the Hub snapshot, the tarball and the 120 extracted members); comparison test mean IoU (majority baseline 0.011): frozen 0.4379 → adapted 0.4302, accuracy (baseline 0.137) 0.5905 → 0.5813, mean class accuracy 0.6239 → 0.6250 (best epoch 4, class-weighted validation loss 0.9368 → 0.9114, validation mean IoU 0.4609 → 0.4451); reload parity mean_iou_diff: 0.0, metrics_identical: True, max_abs_score_diff: 0.0; run summary and executed notebook archived under `.agent/backups/kaggle-e2e-2026-09-19/out/dimer-nb2-prithvi-crop-classification/v2/evidence/` in the workspace |
